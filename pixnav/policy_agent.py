...
class Policy_Agent:
    ...
    def step(self,obs_image,collide=False,early_stop=True,return_debug_image=True):
        ...
        return_distance = distance_pred[0][self.predict_length - 1].detach().cpu().numpy()
        return_goal = np.array(obs_image)
        goal_mask = goal_pred[0][self.predict_length - 1].detach().cpu().numpy()
